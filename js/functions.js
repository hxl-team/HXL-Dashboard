...
function matrixCreation(dataArray)//, typeFilter)
{
    ...
    {
        // correction of bug because empty person counts appeared
        if (dataArray[i]['personCount'].value == "" )
        {
            continue;
        }
        ...
        if (currentDate != dataArray[i]['date'].value.substr(0,10))
        {
            currentDate = dataArray[i]['date'].value.substr(0,10);
            result[currentDate] = '';
        }
    }
    return result;
...
function dataDroping(dateArray, rawData)
{
    ...
    for (var currentDate in dateArray)
    {
        ...
        {
            // correction of bug because empty person counts appeared
            if (rawData[i]['personCount'].value == "" )
            {
                continue;
            }
            
            if (currentDate == rawData[i]['date'].value.substr(0,10))
            {
                var alreadyKnownId = -1;
                for (var j = 0; j < tempPop.length; j++)
                {
                    if (tempPop[j] == rawData[i]['population'].value)
                    {
                        alreadyKnownId = j;
                        break;
                    }
                ...
                if (alreadyKnownId != -1)
                {
                    tempPop[alreadyKnownId] = rawData[i]['population'].value;
                    dailyPopCounts[alreadyKnownId] = rawData[i]['personCount'].value;
                } 
                else
                {
                    tempPop[popIndex] = rawData[i]['population'].value;
                    dailyPopCounts[popIndex] = rawData[i]['personCount'].value;
